#!/usr/bin/env bash
# Uninstalls __APP_ID__: stops + removes its containers and any named
# volumes, deletes its data directory (wherever DATA_DIR points - see
# install.sh) and backups/__APP_ID__/, and removes its .env. Destructive
# - asks for confirmation unless --yes is passed. Safe to re-run (no-op
# on anything already gone).
#
# Usage:
...
#   ./uninstall.sh --keep-backups # keep backups/__APP_ID__/
