#!/usr/bin/env bash
# Uninstalls wikimed: stops + removes its containers and any named
# volumes, deletes its data directory (wherever DATA_DIR points - see
# install.sh) and backups/wikimed/, and removes its .env and
...
#   ./uninstall.sh --keep-backups # keep backups/wikimed/
