...
prompt_if_unset MAPS_WEB_TLS_PORT \
  ...
  "12010" "$ENV_FILE"
prompt_if_unset MAPS_WEB_PORT "Port to publish the map frontend on over plain HTTP (fallback - location marker won't work over this one unless accessed as localhost)" "12011" "$ENV_FILE"
prompt_if_unset MAPS_TILES_PORT "Port to publish the tile server on" "12012" "$ENV_FILE"
...
echo "== offline-maps installed. Visit https://localhost:${MAPS_WEB_TLS_PORT:-12010}/ =="
echo "   (or http://localhost:${MAPS_WEB_PORT:-12011}/ - but the location marker needs the https port above when accessed over LAN)"
